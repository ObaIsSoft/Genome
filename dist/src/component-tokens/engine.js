...
export function buildDecisionVector(creator, genome) {
    ...
    const materialLabel = deriveMaterialLabel([c9[0], c9[1], c9[2]], (c6[2] + 1) / 2 // c6[2] -1..1 → 0..1 for deriveMaterialLabel
    );
    return {
        backdropBlur,
        backdropSaturate,
...
function getComponentRadius(vec, component) {
    const modifier = RADIUS_MODIFIER[component];
    if (modifier === 999)
        return 9999; // full pill/circle → clip via CSS
    const base = vec.radiusBase * (1 + vec.radiusVariance * (modifier - 1));
    return Math.max(0, Math.round(base * modifier));
}
// ── State builder helpers ─────────────────────────────────────────────────────
function buildState(background, backdropFilter, border, boxShadow, borderRadius, transition, transform, typography, opacity, cursor, outline) {
    return {
        background,
        backdropFilter,
        border,
...
function buildComponentTokens(vec, component, primaryHex, surfaceHex, elevatedHex) {
    ...
    const typCtx = (() => {
        if (component === 'badge' || component === 'chip')
            return 'badge';
        if (component === 'nav')
            return 'nav';
        if (component === 'input' || component === 'select' || component === 'textarea')
            return 'input';
    ...
    // ── FILLED variant ──────────────────────────────────────────────────────
    const filled = {
        default: buildState(defaultSurface.background, defaultSurface.backdropFilter, defaultSurface.border, shadows.default, radius, baseTransition, 'none', typography, 1.0, 'pointer', null),
        hover: buildState(hoverSurface.background, hoverSurface.backdropFilter, hoverSurface.border, shadows.hover, radius, hoverTransition, motion.hoverTransform, typography, 1.0, 'pointer', null),
        active: buildState(activeSurface.background, activeSurface.backdropFilter, activeSurface.border, shadows.active, radius, `transform ${motion.activeDurationMs}ms ${vec.easingCurve}, box-shadow ${motion.activeDurationMs}ms ${vec.easingCurve}`, motion.activeTransform, typography, 1.0, 'pointer', null),
        focus: buildState(defaultSurface.background, defaultSurface.backdropFilter, defaultSurface.border, shadows.focus, radius, baseTransition, 'none', typography, 1.0, 'pointer', `0 0 0 2px ${primaryHex}40`),
        disabled: buildState(defaultSurface.background, defaultSurface.backdropFilter, defaultSurface.border, 'none', radius, 'none', 'none', typography, 0.45, 'not-allowed', null),
    };
    // ── GHOST variant ───────────────────────────────────────────────────────
    const ghostBorder = `1px solid ${primaryHex}80`;
    const ghost = {
        default: buildState('transparent', 'none', ghostBorder, flatShadows.default, radius, baseTransition, 'none', typography, 1.0, 'pointer', null),
        hover: buildState(`${primaryHex}12`, 'none', `1px solid ${primaryHex}`, flatShadows.hover, radius, hoverTransition, motion.hoverTransform, typography, 1.0, 'pointer', null),
        active: buildState(`${primaryHex}20`, 'none', `1px solid ${primaryHex}`, flatShadows.active, radius, `transform ${motion.activeDurationMs}ms ${vec.easingCurve}`, motion.activeTransform, typography, 1.0, 'pointer', null),
        focus: buildState('transparent', 'none', `1px solid ${primaryHex}`, flatShadows.focus, radius, baseTransition, 'none', typography, 1.0, 'pointer', `0 0 0 2px ${primaryHex}40`),
        disabled: buildState('transparent', 'none', `1px solid ${primaryHex}40`, 'none', radius, 'none', 'none', typography, 0.45, 'not-allowed', null),
    };
    // ── FLAT variant ────────────────────────────────────────────────────────
    const flat = {
        default: buildState('transparent', 'none', 'none', 'none', radius, baseTransition, 'none', typography, 1.0, 'pointer', null),
        hover: buildState(`${primaryHex}10`, 'none', 'none', 'none', radius, hoverTransition, 'none', typography, vec.hoverOpacity, 'pointer', null),
        active: buildState(`${primaryHex}18`, 'none', 'none', 'none', radius, `opacity ${motion.activeDurationMs}ms ${vec.easingCurve}`, 'none', typography, 1.0, 'pointer', null),
        focus: buildState('transparent', 'none', 'none', flatShadows.focus, radius, baseTransition, 'none', typography, 1.0, 'pointer', `0 0 0 2px ${primaryHex}40`),
        disabled: buildState('transparent', 'none', 'none', 'none', radius, 'none', 'none', typography, 0.40, 'not-allowed', null),
    };
    // ── Rationale ───────────────────────────────────────────────────────────
    const rationale = {
