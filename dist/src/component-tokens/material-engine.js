 ...
 */
export function computeSurface(vec, baseColor, primaryHex, scale = 1.0) {
    ...
    const border = vec.specularHighlight
 ...
 */
export function computeHoverSurface(vec, baseColor, primaryHex) {
    const hoverVec = {
        ...vec,
 ...
 */
export function deriveMaterialLabel(c9, c6_2) {
    const [digital_analog, polished_rough, synthetic_natural] = c9;
    if (digital_analog < -0.45 && polished_rough > 0.30)
        return 'glass';
    if (digital_analog < -0.30 && polished_rough > 0.65)
        return 'metal';
    if (synthetic_natural > 0.40 && c6_2 > 0.40)
        return 'fabric';
    if (c6_2 > 0.45 && synthetic_natural > 0.20)
        return 'paper';
    if (digital_analog < -0.35 && polished_rough < -0.30)
        return 'mineral';
    if (digital_analog > 0.45 && synthetic_natural < -0.20)
        return 'plastic';
    if (digital_analog < -0.55 && polished_rough > 0.10)
        return 'ether';
    return 'standard';
}
