...
export interface SpacingSpec {
  /** CSS padding shorthand — e.g. "12px 20px" or "8px 12px 8px 16px" */
  padding: string;
  /** CSS gap value for flex/grid internal layout */
  gap: string;
  /** px values for individual sides */
  paddingTop: number;
  paddingRight: number;
...
export function deriveSpacing(
  ...
  const verticalMultiplier = (() => {
    if (sem.contentDensity > 0.75) return vec.paddingScale * 0.5;
    if (sem.isTextPrimary)         return vec.paddingScale * 1.6;
    if (sem.isContainer)           return vec.paddingScale * 1.2;
    if (sem.isFeedback)            return vec.paddingScale * 0.6;
    if (sem.isNavigational)        return vec.paddingScale * 0.8;
    if (sem.isEphemeral)           return vec.paddingScale * 0.7;
  ...
  const horizontalMultiplier = (() => {
    if (sem.initiatesAction)       return vec.paddingScale * 2.0;
    if (sem.isNavigational)        return vec.paddingScale * 1.2;
    if (sem.contentDensity > 0.75) return vec.paddingScale * 0.6;
    if (sem.isTextPrimary)         return vec.paddingScale * 1.4;
    if (sem.isContainer)           return vec.paddingScale * 1.3;
    if (sem.isFeedback)            return vec.paddingScale * 0.9;
    return vec.paddingScale * 1.2;
  })();

  // Snap raw px to the nearest multiple of unit
  const snapToGrid = (rawPx: number): number =>
    Math.max(unit, Math.round(rawPx / unit) * unit);
