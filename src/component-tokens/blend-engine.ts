...
const ADDITIVE = ['screen', 'color-dodge', 'lighten', 'luminosity'] as const;

// Subtractive / physical — analog, photographic, material
const SUBTRACTIVE = ['multiply', 'darken', 'color-burn'] as const;

// Atmospheric / layered — painterly, overlapping surfaces
const ATMOSPHERIC = ['overlay', 'soft-light', 'hard-light'] as const;

// Experimental / expressive — high contrast, unexpected
const EXPERIMENTAL = ['difference', 'exclusion', 'hue', 'saturation', 'color'] as const;

// Safe / neutral — reliable, invisible when subtle
const SAFE = ['normal', 'multiply'] as const;
 ...
 */
function pickFromCluster(cluster: readonly string[], t: number): BlendMode {
...
export interface BlendSpec {
  ...
  /** 0–1 element opacity as a design choice */
  elementOpacity: number;
  /** Documentation — describes the blend character */
  blendLabel: string;
}
...
export function deriveBlend(
  ...
  const shouldUseBlend = (
    expressiveScore > 0.4 ||
    experimentalWeight > 0.35 ||
    (vec.usesGradient && sem.containsMedia)
  ) && !sem.isFeedback && sem.elevationLevel < 2;

  let mixBlendMode: BlendMode = 'normal';
