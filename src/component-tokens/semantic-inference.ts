...
import type { ComponentSemantics, ComponentSpec } from '../creator/types.js';
 ...
 */
function score(text: string, signals: string[], weight: number = 1): number {
  const words = new Set(text.split(/[\s\-_/]+/));
  const hits = signals.filter(s => {
    if (s.includes(' ')) return text.includes(s);   // phrase match
 ...
 */
export function inferSemantics(spec: Pick<ComponentSpec, 'name' | 'description'>): ComponentSemantics {
 ...
 */
export function resolveSemantics(spec: ComponentSpec): ComponentSemantics {
  const inferred = inferSemantics(spec);
  if (!spec.semantics) return inferred;
  return { ...inferred, ...spec.semantics } as ComponentSemantics;
}
 ...
 */
export function semanticRadiusModifier(sem: ComponentSemantics): number {
  if (sem.isCircular) return 999; // full 50%

  let modifier = 1.0;

  // Containers with media get slightly more radius (card-like softness)
  if (sem.isContainer && sem.containsMedia) modifier += 0.20;
  // Ephemeral elements (tooltips, toasts) are compact — less radius
  if (sem.isEphemeral && !sem.isContainer) modifier -= 0.40;
  // Dense data elements stay sharp
  if (sem.contentDensity > 0.7) modifier -= 0.25;
  // Navigational items compress slightly
  if (sem.isNavigational) modifier -= 0.15;
  // Feedback elements (badges, alerts) are compact
  if (sem.isFeedback && sem.contentDensity < 0.4) modifier -= 0.30;
  // High visual weight elements soften slightly
  if (sem.visualWeight > 0.7) modifier += 0.15;
  // Action-initiating buttons get their full radius
  if (sem.initiatesAction) modifier = Math.max(modifier, 0.85);
 ...
 */
export function semanticLiftModifier(sem: ComponentSemantics): number {
  if (!sem.initiatesAction && sem.interactivity < 0.3) return 0;
  if (sem.isTextPrimary && !sem.isContainer) return 0;
  if (sem.contentDensity > 0.8) return 0; // data tables don't lift

  let modifier = sem.interactivity;

  // Containers lift more (cards, product tiles)
  if (sem.isContainer && sem.containsMedia) modifier *= 1.3;
  // Navigational items lift subtly
  if (sem.isNavigational) modifier *= 0.5;
  // Feedback elements don't lift
  if (sem.isFeedback) modifier *= 0.1;
  // Ephemeral elements already elevated — no lift
  if (sem.isEphemeral) modifier = 0;
 ...
 */
export function semanticShadowScale(sem: ComponentSemantics, baseScale: number): number {
 ...
 */
export function semanticPaddingModifier(sem: ComponentSemantics): number {
  if (sem.contentDensity > 0.75) return 0.6;  // compact
  if (sem.isTextPrimary)         return 1.4;  // editorial breathing room
  if (sem.isContainer)           return 1.2;  // cards need space
  if (sem.isFeedback)            return 0.7;  // badges/alerts are tight
  if (sem.isNavigational)        return 0.9;
  return 1.0;
}
