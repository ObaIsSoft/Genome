...
export interface ComponentTypography {
  /** CSS letter-spacing value e.g. "0.023em" */
  letterSpacing: string;
  /** font-variant-numeric value or null */
  fontVariantNumeric: string | null;
  /** text-wrap value for headings */
  textWrap: string | null;
  /** text-transform for uppercase label contexts */
  textTransform: string | null;
  /** font-size modifier multiplier (1.0 = no change) */
  fontSizeScale: number;
  /** line-height override for this context, or null (inherit) */
  lineHeight: string | null;
}

type TypographyContext = 'button' | 'badge' | 'label' | 'body' | 'heading' | 'caption' | 'nav' | 'input' | 'code';
 ...
 */
const LETTER_SPACING_MODIFIER: Record<TypographyContext, number> = {
  button:  0.010,
  badge:   0.040,
  label:   0.060,
  body:    0.000,
 ...
 */
const UPPERCASE_ELIGIBLE: Set<TypographyContext> = new Set(['badge', 'label', 'nav', 'caption']);
 ...
 */
export function getFontFeatureSettings(vec: ComponentDecisionVector): string | null {
  const features: string[] = [];

  if (vec.tabulaNumeric) features.push('"tnum"');

  // Oldstyle figures for body text in warm/analog genomes
  const isAnalogWarm = vec.surfaceWarmth > 0.55 && vec.backdropBlur === 0;
  if (isAnalogWarm) features.push('"onum"');

  // Discretionary ligatures for highly expressive authorial voices
  if (vec.letterSpacingBase < -0.005) features.push('"dlig"');
 ...
 */
export function formatSignatureNumber(value: number, separator: string): string {
