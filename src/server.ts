                ...
                switch (request.params.name) {
                    case "generate_design_genome": {
                        ...
                        const organismsDefinition = await this.extractor.analyzeOrganisms(
                            args.intent, detectedSector, estimatedCounts, ecoBiomeContextGen
                        );
                        ...
                        const eco = ecosystemGenerator.generate(seed, traits, {
                            ...
                            const ecoOut: EcosystemResponse = {
                                ...
                                civilizationGap: parseFloat(
                                    Math.max(0, 0.81 - (eco.evolution?.complexity || 0)).toFixed(3)
                                )
                            };
                    ...
                    case "generate_civilization": {
                        ...
                        if (tier.civilizationGenome?.chromosomes) {
                            ...
                            for (const ch of Object.keys(tier.civilizationGenome.chromosomes)) {
                                if (l3Registry.has(ch)) civTracker.track(ch);
                            }
                        }
                    ...
                    case "generate_page_composition": {
                        ...
                        if (args.design_brief) {
                            const brief = args.design_brief as { thesis?: string; mandates?: string[]; antiPatterns?: string[] };
                            if (brief.thesis) {
                    ...
                    case "generate_component_tokens": {
                        ...
                        if (!designGenome.chromosomes || !designGenome.chromosomes.ch5_color_primary) {
                            throw new McpError(
                                ErrorCode.InvalidParams,
                                "genome must be a complete L1 Design Genome with chromosomes (from generate_design_genome or generate_design_through_persona). Do not pass a Creator Genome here."
                            );
                        }

                        // Derive or use provided creator genome
                        let creatorGenome: any;
                        ...
                        let targetComponents: string[] | undefined;
                        if (args.components && Array.isArray(args.components) && args.components.length > 0) {
                            targetComponents = args.components.filter((c: string) =>
                                (ALL_COMPONENTS as readonly string[]).includes(c)
                            );
                        }

                        const tokenMap = generateComponentTokens(creatorGenome, designGenome, {
                            components: targetComponents as any,
                        });
